...
uses windows, LCLIntf, classes, symbolhandler, CEFuncProc,NewKernelHandler;
...
type
  ...
  TPointerTable=record
    case integer of
      1: (pointeraddress: PPointerAddress); //if this is the last level (maxlevel) this is an PPointerList
...
type
  ...
  TAddressTable=record
    case integer of
      1: (addressvalue: pointer); //if this is the last level (maxlevel) this is a pointer to a type of the value to scan for (4 byute and single are 4 byte, double is 8 byte)
...
type TRescanHelper=class
  ...
  public
    ...
    function AddPointer(Address: ptrUint; value: ptrUint): PPointerAddress;
    function findPointer(Address: ptrUint): PPointerAddress;

    function AddAddress(Address: ptrUint; value: pointer; valuesize: integer): pointer;
    function findAddress(Address: ptrUint): pointer;
...
function TRescanHelper.findPointer(Address: ptrUint): PPointerAddress;
{
...
function TRescanHelper.AddPointer(Address: ptrUint; value: ptrUint): PPointerAddress;
...
function TRescanHelper.findAddress(Address: ptrUint): pointer;
var
  level: integer;
  entrynr: integer;
...
function TRescanHelper.AddAddress(Address: ptrUint; value: pointer; valuesize: integer): pointer;
var
  level: integer;
  entrynr: integer;
